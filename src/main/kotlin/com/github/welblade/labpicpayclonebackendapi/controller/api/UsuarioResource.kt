...
import com.github.welblade.labpicpayclonebackendapi.controller.swagger.UsuarioResourceDoc
...
) : ResourceBase<UsuarioDto>(), UsuarioResourceDoc {

    @GetMapping("/{login}")
    override fun encontrar(@PathVariable login: String) : ResponseEntity<UsuarioDto>{
        val usuario = usuarioService.findByLoginDto(login)
        return sucessoRetornaItem(usuario)
    }

    @GetMapping("/contatos")
    override fun listarContatos(@RequestParam login: String) : ResponseEntity<List<UsuarioDto>>{
